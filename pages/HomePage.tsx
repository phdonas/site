...
const HomePage: React.FC = () => {
  ...
  return (
    <main>
      {config.hero.visible !== false && <Hero />}
      {config.sections?.pillars?.visible !== false && <PillarGrid />}
      {config.sections?.services?.visible !== false && <ServicesSection />}
      
      {config.sections?.videos?.visible !== false && (
        <section className="py-24 px-6 bg-black text-white overflow-hidden">
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
              <div className="max-w-xl">
                <h2 className="text-4xl md:text-5xl font-bold mb-4 tracking-tight">{config.sections?.videos?.title || "Aulas Curtas & Insights"}</h2>
                <p className="text-gray-400 text-lg font-medium">{config.sections?.videos?.subtitle || "Conteúdo visual direto ao ponto para sua evolução."}</p>
              </div>
              <a href="#/artigos" className="text-blue-500 font-bold flex items-center gap-2 hover:underline">
                {config.sections?.videos?.linkText || "Ver todos os vídeos"} <Play size={16} />
              </a>
            </div>
      ...
      {config.sections?.articles?.visible !== false && <ArticleGallery limit={6} />}
      {config.sections?.leadForm?.visible !== false && <LeadForm />}
    </main>
  );
};
